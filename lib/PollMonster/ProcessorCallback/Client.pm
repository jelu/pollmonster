...
package PollMonster::ProcessorCallback::Client;
...
sub new {
    ...
    $args{on_connect} = sub {
        ...
        unless ($name eq PROCESSOR_CALLBACK_NAME) {
            if (exists $self->{on_error}) {
                $self->{on_error}->($self, 'connected to wrong service, expected '.PROCESSOR_CALLBACK_NAME.' but got '.$name);
            }
            return;
        }
...
sub result {
    ...
    return $self->{rpc}->call('result', $result, $done);
}
...
sub error {
    ...
    return $self->{rpc}->call('error', $errno, $errstr);
}
