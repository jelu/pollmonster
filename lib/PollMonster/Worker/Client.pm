...
package PollMonster::Worker::Client;
...
sub new {
    ...
    $args{on_connect} = sub {
        ...
        unless ($name eq WORKER_NAME) {
            if (exists $self->{on_error}) {
                $self->{on_error}->($self, 'connected to wrong service, expected '.WORKER_NAME.' but got '.$name);
            }
            return;
        }
    ...
    $args{on_service} = sub { $self; };

    unless (exists $args{service}) {
        $args{service} = {};
    }

    foreach my $callback (qw(ready result_ready)) {
...
sub work {
    ...
    if (defined $private) {
        return $self->{rpc}->call('work', $uuid, $module, $payload, $private);
    }

    return $self->{rpc}->call('work', $uuid, $module, $payload);
}
...
=head2
    sub _ready {
        ...
        unless (defined $_dispatcher[$id] and defined $_rpc[$id]) {
            return;
        }
        ...
        if (($_payload[$id] = $_dispatcher[$id]->dequeue)) {
            my ($err) = $_rpc[$id]->call('work', @{$_payload[$id]});
            if (defined $err) {
                $_dispatcher[$id]->requeue($_payload[$id]);
                $_payload[$id] = undef;
            }
        }
        else {
            $_dispatcher[$id]->wait_for_work($self);
        }
