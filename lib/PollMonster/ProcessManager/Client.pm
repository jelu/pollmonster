...
package PollMonster::ProcessManager::Client;
...
sub new {
    ...
    $args{on_connect} = sub {
        ...
        unless ($name eq PROCESS_MANAGER_NAME) {
            if (exists $self->{on_error}) {
                $self->{on_error}->($self, 'connected to wrong service, expected '.PROCESS_MANAGER_NAME.' but got '.$name);
            }
            return;
        }
    ...
    foreach my $callback (qw(register unregister list)) {
...
sub register {
    my ($self, $service, $uri, $pid) = @_;

    unless (defined $service and defined $uri and defined $pid) {
    ...
    return $self->{rpc}->call('register', PollMonster->CFG(GLOBAL_NAME, 'hostname', Sys::Hostname::hostname), $service, $uri, $pid);
}
...
sub unregister {
    ...
    return $self->{rpc}->call('unregister', PollMonster->CFG(GLOBAL_NAME, 'hostname', Sys::Hostname::hostname), $service, $uri, $pid);
}
...
sub list {
    ...
    if (defined $type) {
        return $self->{rpc}->call('list', PollMonster->CFG(GLOBAL_NAME, 'hostname', Sys::Hostname::hostname), $type);
    }

    return $self->{rpc}->call('list', PollMonster->CFG(GLOBAL_NAME, 'hostname', Sys::Hostname::hostname));
}
...
sub heartbeat {
    ...
    return $self->{rpc}->call('heartbeat', PollMonster->CFG(GLOBAL_NAME, 'hostname', Sys::Hostname::hostname), $service, $uri, $pid);
}
