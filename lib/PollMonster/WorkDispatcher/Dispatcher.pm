...
package PollMonster::WorkDispatcher::Dispatcher;
...
sub add_worker {
    $_[0]->{worker}->{$_[1]} = $_[2] if (defined $_[1] and defined $_[2]);

    $_[0];
}

sub del_worker {
    return delete $_[0]->{worker}->{$_[1]} if (defined $_[1]);

    return;
}

sub set_worker {
    $_[0]->{worker} = $_[1] if (defined $_[1]);

    $_[0];
}

sub num_workers {
    int(scalar %{$_[0]->{worker}});
}
