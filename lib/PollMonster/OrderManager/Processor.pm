...
package PollMonster::OrderManager::Processor;
...
sub add_order {
    $_[0]->{order}->{$_[1]} = $_[2] if (defined $_[1] and defined $_[2]);

    $_[0];
}

sub del_order {
    return delete $_[0]->{order}->{$_[1]} if (defined $_[1]);

    return;
}

sub set_order {
    $_[0]->{order} = $_[1] if (defined $_[1]);

    $_[0];
}

sub num_orders {
    int(scalar %{$_[0]->{order}});
}
