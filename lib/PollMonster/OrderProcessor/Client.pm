...
package PollMonster::OrderProcessor::Client;
...
sub new {
    ...
    $args{on_connect} = sub {
        ...
        unless ($name eq ORDER_PROCESSOR_NAME) {
            if (exists $self->{on_error}) {
                $self->{on_error}->($self, 'connected to wrong service, expected '.ORDER_PROCESSOR_NAME.' but got '.$name);
            }
            return;
        }
    ...
    foreach my $callback (qw(add remove list result)) {
...
sub add {
    my ($self, $uuid, $module, $parameter, $option) = @_;

    unless (defined $uuid and defined $module and ref($parameter) eq 'HASH') {
    ...
    return $self->{rpc}->call('add', $uuid, $module, $parameter, $option);
}
...
sub result {
    my ($self, $uuid, $module, $payload) = @_;

    unless (defined $self->{rpc}) {
        return;
    }

    return $self->{rpc}->call('result', $uuid, $module, $payload);
}
