...
package PollMonster::APIBridge::JSONP;
...
sub new {
    ...
    $args{name} = APIBRIDGE_JSONP_NAME;
    $self->{rpc} = PollMonster::RPC::Server->new(%args);

    if ($args{jsonp_uri} =~ /^http:\/\/([a-zA-Z0-9\.-]+)(?::([0-9]+))*/o) {
        my ($host, $port) = ($1, $2);
        ...
        $self->{socket} = AnyEvent::Socket::tcp_server $host, $port, sub {
                ...
                on_read => sub {
                    ...
                    if ($self->{client}->{$handle}->{rbuf} =~ /\r\n\r\n$/o) {
                        Coro::async_pool {
                            ...
                            my $query = $request->uri->query_form_hash;

                            if (!defined $query->{callback}) {
                                $response->code(HTTP_BAD_REQUEST);
                                $response->content('No Callback');
                            }
                            elsif ($query->{callback} =~ /\W/o) {
                                $response->code(HTTP_BAD_REQUEST);
                                $response->content('Invalid Callback');
                            }
                            else {
                                if (!defined $query->{method}) {
                                    ...
                                    unless ($response->code) {
                                        ...
                                        if (defined $errno) {
                                            ...
                                            $response->content(sprintf('%s(%s);', $query->{callback}, (defined $r ? $r : '')));
                                        }
                                    }
                                }
                            }
            ...
            PollMonster::INFO and $self->{logger}->info('JSON-P server ready at http://', $host, ':', $port);
