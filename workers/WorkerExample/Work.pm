...
sub set_num_elements {
    $_[0]->{num_elements} = $_[1] if (defined $_[1]);

    $_[0];
}

sub TO_JSON {
    if (defined $_[0]->{num_elements}) {
        my @elements;

        push(@elements, 'example') for (1 .. $_[0]->{num_elements});

        return [ $_[0]->{sent}, \@elements ];
    }
