...
use WorkerExample::Work;
use WorkerExample::Result;
...
PollMonster::ModuleFactory->instance->register_worker(
    'example',
    { new => sub {
        return WorkerExample::Work->new(sent => Time::HiRes::time);
      },
      verify => sub {
          my ($payload) = @_;

          if ($payload->isa('WorkerExample::Work')) {
              return 1;
          }
      ...
      to_result => sub {
          ...
          return WorkerExample::Result->new(sent => $payload->[0], received => $payload->[1]);
      },
      run => sub {
          my ($payload) = @_;

          if (exists $payload->[1]) {
              return [ $payload->[0], Time::HiRes::time, $payload->[1] ];
          }

          [ $payload->[0], Time::HiRes::time ];
      }
    });
